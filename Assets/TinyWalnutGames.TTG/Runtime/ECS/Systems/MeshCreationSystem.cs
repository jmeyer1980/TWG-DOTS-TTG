...
namespace TinyWalnutGames.TTG.TerrainGeneration
{
    ...
    /// Handles URP/BRP material compatibility and conditional cleanup for test environments.
    /// </summary>
    ...
    public partial class MeshCreationSystem : SystemBase
    {
        ...
        protected override void OnUpdate()
        {
            ...
            Entities
                ...
                .ForEach((Entity entity, ref TerrainGenerationState generationState, in MeshDataComponent meshData) =>
                {
                    ...
                    // CONDITIONAL CLEANUP: Only dispose blob assets in production builds
                    // This preserves components in test environments for verification
                    #if !UNITY_INCLUDE_TESTS
                    // CRITICAL FIX: Dispose blob assets after conversion to Unity Mesh
                    // This prevents memory leaks by cleaning up ECS data once it's no longer needed
                    DisposeMeshDataBlobAssets(meshData);
                    ecb.RemoveComponent<MeshDataComponent>(entity);
                    
                    // ENHANCED CLEANUP: Also clean up authoring blob assets once mesh is created
                    if (EntityManager.HasComponent<TerraceConfigData>(entity))
                    {
                        var terraceConfig = EntityManager.GetComponentData<TerraceConfigData>(entity);
                        DisposeTerraceConfigBlobAsset(terraceConfig);
                    ...
                    if (EntityManager.HasComponent<TerrainMaterialData>(entity))
                    {
                        ...
                        DisposeMaterialDataBlobAsset(materialData);
                        ecb.RemoveComponent<TerrainMaterialData>(entity);
                    }
                    
                    // AGGRESSIVE CLEANUP: Force immediate garbage collection after mesh creation
                    System.GC.Collect();
                    System.GC.WaitForPendingFinalizers();
                    
                    // DOUBLE-CLEANUP: Force another GC pass to ensure everything is cleaned up
                    System.GC.Collect();
                    #endif
                    
                    Debug.Log($"Terrain generation completed for entity {entity}. Created mesh entity {meshEntity} with GameObject '{gameObject.name}' containing {mesh.vertexCount} vertices and {mesh.triangles.Length / 3} triangles.");
                }).WithStructuralChanges().Run();
                
            ecb.Playback(EntityManager);
            ecb.Dispose();
            
            // CONDITIONAL CLEANUP: Only run ultra-aggressive cleanup in production builds
            #if !UNITY_INCLUDE_TESTS
            // ULTRA-AGGRESSIVE: Additional cleanup pass after all entities processed
            CleanupAllAuthoringBlobAssets();
            #endif
        }
        
        private void CleanupAllAuthoringBlobAssets()
        {
            var ecb = new EntityCommandBuffer(Allocator.TempJob);
            
            // Emergency cleanup: Clean up any remaining terrace config data on completed entities
            Entities
                .WithAll<TerraceConfigData, GeneratedTerrainMeshTag>()
                .ForEach((Entity entity, in TerraceConfigData terraceConfig) =>
                {
                    // If entity has both blob asset and generated mesh tag, the blob asset is no longer needed
                    DisposeTerraceConfigBlobAsset(terraceConfig);
                    ecb.RemoveComponent<TerraceConfigData>(entity);
                    Debug.Log($"EMERGENCY-CLEANUP: Disposed terrace config for entity {entity}");
                }).WithoutBurst().Run();
                
            // Emergency cleanup: Clean up any remaining material data on completed entities
            Entities
                .WithAll<TerrainMaterialData, GeneratedTerrainMeshTag>()
                .ForEach((Entity entity, in TerrainMaterialData materialData) =>
                {
                    // If entity has both blob asset and generated mesh tag, the blob asset is no longer needed
                    DisposeMaterialDataBlobAsset(materialData);
                    ecb.RemoveComponent<TerrainMaterialData>(entity);
                    Debug.Log($"EMERGENCY-CLEANUP: Disposed material data for entity {entity}");
                }).WithoutBurst().Run();
        ...
        private Material[] GetMaterialsForEntity(Entity entity)
        {
            ...
            if (EntityManager.HasComponent<TerrainMaterialData>(entity))
            {
                ...
                for (int i = 0; i < materialData.MaterialCount; i++)
                {
                    ...
                    if (instanceID != 0)
                    {
                        ...
                        materials[i] = GetMaterialFromRegistry(instanceID) ?? GetDefaultMaterial();
#endif
                    }
                    else
                    {
        ...
        private void InitializeMaterialRegistry()
        {
            ...
            Debug.Log($"MeshCreationSystem: Initialized material registry with {materialRegistry.Count} materials for runtime builds");
        }
        ...
        private Entity CreateMeshEntity(EntityCommandBuffer ecb, GameObject meshGameObject)
        {
            ...
            // Add a tag component to identify this as a generated terrain mesh
            ecb.AddComponent<GeneratedTerrainMeshTag>(meshEntity);
        ...
        private static void DisposeTerraceConfigBlobAsset(TerraceConfigData terraceConfig)
        {
            try
            {
            ...
            {
                // Blob asset already disposed - this is actually expected behavior in ECS
                // No need to log warnings for normal disposal flow
            }
        }
        
        private static void DisposeMaterialDataBlobAsset(TerrainMaterialData materialData)
        {
            try
            {
        ...
        protected override void OnDestroy()
        {
            ...
            Entities
                ...
                .ForEach((Entity entity, in MeshGameObjectReference meshRef) =>
                {
                    ...
                    {
                        Debug.LogWarning($"Failed to cleanup GameObject for entity {entity}: {ex.Message}");
                    }
#endif
                }).WithoutBurst().Run();
